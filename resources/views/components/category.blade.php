<div class="flex gap-4 justify-center">
    <form action="/blog" method="GET">
        ...
        <div class="relative">
            <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                <svg class="w-4 h-4 text-body" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" m
    ...
    <el-dropdown class="inline-block">
        <button
            class="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-[#1e2939] px-3 py-3 text-sm font-semibold text-white inset-ring-1 inset-ring-white/5 hover:bg-gray-600">
            Kategori
            ...
            <div class="py-1">
                @foreach ($categories as $category)
                    <a href="/blog?category={{ $category->category_name }}"
                        class="block px-4 py-2 text-sm text-gray-300 focus:bg-white/5 focus:text-white focus:outline-hidden">{{ $category['category_name'] }}</a>
                @endforeach
