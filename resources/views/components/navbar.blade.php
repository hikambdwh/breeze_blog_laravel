                    ...
                    @if (Auth::check())
                        <el-dropdown class="relative ml-3">
                            ...
                            <button
                                ...
                                <img src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : '/img/avatar-default.png' }}"
                                    alt=""
                                    class="size-8 rounded-full outline -outline-offset-1 outline-white/10" />
                                <p class="px-3 py-2 text-sm font-medium text-gray-300">{{ Auth::user()->name }}</p>
                            </button>
                            ...
                            <el-menu anchor="bottom end" popover
                                ...
                                <a href="/profile"
                                    class="block px-4 py-2 text-sm text-gray-700 focus:bg-gray-100 focus:outline-hidden">Your profile</a>
                                <a href="/dashboard"
                                    class="block px-4 py-2 text-sm text-gray-700 focus:bg-gray-100 focus:outline-hidden">Settings</a>
                                <form action="/logout" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="block px-4 py-2 text-sm text-gray-700 focus:bg-gray-100 focus:outline-hidden cursor-pointer">Sign
                                        out</button>
                                </form>
                            </el-menu>
                        </el-dropdown>
    ...
    <el-disclosure id="mobile-menu" hidden class="block md:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
            ...
            <a href="/" aria-current="page"
                class="block rounded-md bg-gray-900 px-3 py-2 text-base font-medium text-white">Laravel</a>
            <a href="/home"
